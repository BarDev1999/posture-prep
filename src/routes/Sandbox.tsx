...
export default function Sandbox() {
  ...
  return (
    <div className="mx-auto w-full max-w-2xl px-4 py-4">
      <h1 className="eyebrow">Query sandbox</h1>
      ...
      <label className="mt-4 block">
        ...
        <select
          ...
          className="mt-1 min-h-12 w-full rounded-sm border border-rule bg-raised px-2 text-sm"
        >
      ...
      {exercise ? (
        <div className="mt-3 sheet p-3">
          <div className="flex flex-wrap gap-2 data">
        ...
        <p className="mt-3 sheet p-3 text-sm text-muted">
      ...
      <label className="mt-3 block">
        ...
        <textarea
          ...
          autoComplete="off"
          placeholder="SELECT ..."
          className="mt-1 w-full resize-y rounded-sm border border-rule bg-raised p-3 font-mono text-sm leading-relaxed"
        />
      </label>
      ...
      <div className="flex gap-2">
        <button
          ...
          className="min-h-12 flex-1 rounded-sm bg-accent px-4 text-sm font-semibold text-accent-ink disabled:opacity-50"
        >
        ...
        {dirty ? (
          <button
            ...
            className="min-h-12 rounded-sm border border-rule px-3 text-sm text-muted"
          >
            Reset data
          </button>
        ) : null}
      ...
      {outcome && !outcome.ok ? (
        <div className="mt-4 border border-critical/50 bg-sheet p-3">
      ...
      {outcome && outcome.ok ? (
        <div className="mt-4 space-y-3">
          ...
          <p className="data">{outcome.elapsedMs.toFixed(1)} ms</p>
        </div>
      ) : null}
  ...
  return (
    <details className="mt-3 sheet">
      ...
      <div className="border-t border-rule px-3 pb-3">
        <p className="mt-2 data">{content.sqlSchemaTables.join(', ')}</p>
        <Markdown>{'```sql\n' + content.sqlSchema + '\n```'}</Markdown>
      </div>
    </details>
  )
...
function Feedback({
  ...
  return (
    <div className={`mt-4 border bg-sheet p-3 ${tone}`}>
